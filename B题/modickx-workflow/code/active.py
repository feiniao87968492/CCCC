...
from params import COORD_ABS_LIMIT, R_MIN, SPEED
...
def _dedup_pts(pts: list[np.ndarray]) -> list[np.ndarray]:
    out: list[np.ndarray] = []
...
def _path_length(x: np.ndarray, order: list[np.ndarray]) -> float:
    if not order:
        return 0.0
    L = float(np.linalg.norm(order[0] - x))
    for i in range(len(order) - 1):
        L += float(np.linalg.norm(order[i + 1] - order[i]))
    return L


def _held_karp_open(x: np.ndarray, arr: list[np.ndarray]) -> float:
    n = len(arr)
    inf = float("inf")
    dp = [[inf] * n for _ in range(1 << n)]
    for j in range(n):
        dp[1 << j][j] = float(np.linalg.norm(arr[j] - x))
    for mask in range(1 << n):
        for j in range(n):
            ...
            for i in range(n):
                if not (prev & (1 << i)):
                    continue
                cand = dp[prev][i] + float(np.linalg.norm(pj - arr[i]))
                if cand < best:
                    best = cand
            dp[mask][j] = best
    return min(dp[(1 << n) - 1])
...
def _nn_tour(x: np.ndarray, arr: list[np.ndarray]) -> list[np.ndarray]:
    left = list(range(len(arr)))
    tour: list[np.ndarray] = []
    cur = x
    while left:
        j = min(left, key=lambda i: float(np.linalg.norm(arr[i] - cur)))
        tour.append(arr[j])
        cur = arr[j]
        left.remove(j)
    return tour
...
def _two_opt(x: np.ndarray, tour: list[np.ndarray]) -> list[np.ndarray]:
    if len(tour) < 4:
        return tour
    improved = True
    while improved:
        ...
        n = len(tour)
        for i in range(n - 1):
            a = x if i == 0 else tour[i - 1]
            b = tour[i]
            for k in range(i + 1, n):
                c = tour[k]
                d = tour[k + 1] if k + 1 < n else None
                before = float(np.linalg.norm(b - a)) + (0.0 if d is None else float(np.linalg.norm(d - c)))
                after = float(np.linalg.norm(c - a)) + (0.0 if d is None else float(np.linalg.norm(d - b)))
...
def _p4_snake_path(x: np.ndarray, arr: list[np.ndarray]) -> list[np.ndarray] | None:
    ...
    candidates = [
        snake[i0:] + snake[:i0],
        list(reversed(snake[i0:] + snake[:i0])),
    ...
    best = min(candidates, key=_len)
    return [pts[i].copy() for i in best]


def shortest_path_length(x: np.ndarray, pts: list[np.ndarray]) -> float:
    """Open path length from x through pts.

    k<=8: exact Held-Karp; 8<k<=20: NN+2-opt; k>20: P4 snake if on-grid else NN+2-opt.
    """
    if not pts:
        return 0.0
    ...
    if k == 0:
        return 0.0
    if k == 1:
        return float(np.linalg.norm(arr[0] - x))
    if k <= 8:
        return _held_karp_open(x, arr)
    if k <= 20:
        return _path_length(x, _two_opt(x, _nn_tour(x, arr)))
    snake = _p4_snake_path(x, arr)
    if snake is not None:
        return _path_length(x, snake)
    return _path_length(x, _two_opt(x, _nn_tour(x, arr)))


def route_length_estimate(x: np.ndarray, pts: list[np.ndarray]) -> float:
    """Ranking-time path cost: exact for k<=8, O(k) snake/NN otherwise. No per-candidate 2-opt."""
    if not pts:
        return 0.0
    x = np.asarray(x, dtype=float).reshape(2)
    arr = _dedup_pts(pts)
    ...
    if k == 0:
        return 0.0
    if k <= 8:
        return shortest_path_length(x, arr)
    snake = _p4_snake_path(x, arr)
    if snake is not None:
        return _path_length(x, snake)
    return _path_length(x, _nn_tour(x, arr))
...
def _rank_tuple(q, s, bearing, x, verts, samples, remaining_cover, receptions) -> tuple:
    ...
    L = float(np.linalg.norm(q - x)) + route_length_estimate(q, rest)
