...
class Q4StateTests(unittest.TestCase):
    ...
    def test_near_is_detected_not_absent(self):
        st = Q4State()
        st.apply_measure(8, np.zeros(2), "near")
        self.assertEqual(st.channels[8].status, "detected")
        self.assertTrue(st.channels[8].ever_detected)

    def test_direction_requires_finite_angle(self):
        st = Q4State()
        with self.assertRaises(ValueError):
            st.apply_measure(9, np.zeros(2), "direction", svd_deg=float("nan"))
        self.assertEqual(st.channels[9].status, "unseen")

    def test_clear_fail_stays_detected(self):
        st = Q4State()
        st.apply_measure(10, np.zeros(2), "direction", svd_deg=45.0)
        st.apply_clear(10, np.array([10.0, 0.0]), False)
        self.assertEqual(st.channels[10].status, "detected")
        self.assertFalse(st.channels[10].cleared)

    def test_clear_success_is_cleared(self):
        st = Q4State()
        st.apply_measure(11, np.zeros(2), "near")
        st.apply_clear(11, np.zeros(2), True)
        self.assertEqual(st.channels[11].status, "cleared")
