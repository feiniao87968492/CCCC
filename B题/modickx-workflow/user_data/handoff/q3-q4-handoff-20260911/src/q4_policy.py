...
def _candidate_points(state: Q4State, *, certificate_mode: bool = False) -> list[np.ndarray]:
    ...
    for ch in state.channels.values():
        ...
        elif ch.status == "detected":
            rem = ch.remaining_p4()
            if certificate_mode or ch.halfplane_failed:
                need.update(rem)
            elif ch.first_site is not None and ch.first_bearing is not None:
                half = [i for i in rem if in_bearing_halfplane(pts[i], ch.first_site, ch.first_bearing)]
                need.update(half if half else rem)
            else:
                need.update(rem)
    if not need:
        return []
