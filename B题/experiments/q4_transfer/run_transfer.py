"""Run new Q4 families using the immutable first-round scenario/event engine."""
import argparse
import csv
...
ROOT=HERE.parents[1]
sys.path[:0]=[str(HERE),str(ROOT/'experiments/q4_ablation')]
import run_ablation as engine
from q4_transfer_policies import VARIANTS,make_runner,cover_context
...
def settings(suite):
    if suite=='primary':
        start,count=30000,200
        extras=50
    else:
        start,count=20000,30
        extras=10
    ...
    if suite=='dev':
        template=[s for s in template if s['source_kind']=='boundary']
...
def main():
    ...
    dependencies=[HERE/'q4_transfer_policies.py',Path(__file__),HERE/'design.md',
                  HERE/'reviews/implementation-readiness.md',
                  ROOT/'experiments/q3_ablation/policies.py',ROOT/'experiments/q3_ablation/evaluation/summary.csv']
    manifest['sha256'].update({str(p.relative_to(ROOT)):engine.sha(p) for p in dependencies})
    manifest['review_status']='implementation-readiness PASS_WITH_WARNINGS; result review pending'
