...
6. Geometry/combination tuning: 1900/1950/1980 m outer witnesses, optimistic
...
## Development selection and primary lock

Initial dev40: 24 x 40 = 960 runs; all completed, no source drift. Baseline
mean T/K 567.771; S 561.455; D-optimal 564.707; JSO compact 560.702;
unmodified JSO 568.655; insertion80/200/400 all worsened the mean. Based only
on this development evidence, add four combinations: S_doptimal,
SO_doptimal, S_dopt_short, JSO_compact_dopt. Retain every initial arm,
including all losing arms and inactive optical2, in the primary matrix.
The final registry has 28 arms and no further tuning will use primary data.
Additional development run uses the same 40 settings for four combinations
plus baseline; its purpose is execution checking, not selective omission.

Initial dev40 manifest recorded previous_best's radius but omitted its route,
reuse and clear_rho settings. Its snapshotted make_runner delegates exactly to
OLD_VARIANTS['proxy_reuse_early_compact']; actual execution was unaffected.
An additive provenance-correction.json records the effective configuration;
the original manifest is preserved. Registry metadata is corrected before
the additional development run and primary batch. Frozen baseline is unchanged.
