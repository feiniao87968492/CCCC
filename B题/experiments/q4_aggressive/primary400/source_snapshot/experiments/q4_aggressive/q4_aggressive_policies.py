...
VARIANTS={
    ...
    'ER':_cfg('ER',family='E+R',e_ratio=1.,r_count=2),
    'EN':_cfg('EN',family='E+N',e_ratio=1.,n_mass=.20),
    'ERN':_cfg('ERN',family='E+R+N',e_ratio=1.,r_count=2,n_mass=.20),
}
...
class AggressiveRunner(base.LocalRunner):
    ...
    def _has(self,family):
        return family in self.config.family.split('+')
    ...
    def _refinement_points(self,k,region):
        ...
        if not self._has('B') or not points:return points
    ...
    def _optical_fallback(self,k,region):
        if not self._has('E') or len(optical_cover_points(region['verts']))>256:
    ...
    def _adaptive_task(self,k):
        ...
        if self._has('R') and self.state.channels[k].extra_measures>=self.config.r_count:
        ...
        if self._has('H') and region['rho']<=self.config.h_rho:
            ...
            if est is not None and version not in self._h_used[k] and len(self._h_used[k])<2 and all(np.linalg.norm(est-p)>1 for p in self.failed_clear_points[k]):
                return est,'h_clear'
        if self._has('E') and region['rho']<=160 and len(optical_cover_points(region['verts']))<=48 and kind in ('measure','aggr_clear','mass_clear'):
            return region['center'],'optical'
        return q,kind
    ...
    def _g_endpoint(self,p):
        if not self._has('G') or self._g_guard:return
        self._g_guard=True
        try:
    ...
    def _n_endpoint(self,p):
        if not self._has('N') or self._in_optical:return
        for k in list(self.state.pending_detected()):
