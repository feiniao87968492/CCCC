...
Combination lock is selection.json, generated only from audited dev_valid40:
G075 (-5.429715), R2 (-0.618118), N20 (-0.016437) s/source versus LC on the
general30 settings. N20 wins its exact tie with N50 lexicographically. Add
GR=G075+R2, GN=G075+N20, GRN=G075+R2+N20. All initial16 arms remain in
the primary matrix, for19 arms total. A separate40-setting combination batch
includes all four controls and G075/R2/N20 to check matched reproducibility.
These small development gains are selection evidence, not primary conclusions.

The provisional
ER/EN/ERN selection is withdrawn: its runs did not implement the declared
E/B/G mechanisms and reused older seeds. See invalid-runs.md; none of those
statistics may inform corrected selection or final efficiency conclusions.
...
require `K == N` for a setting to be eligible. Each arm passing this predicate
on every setting is eligible for ranking on350 settings: mixed endpoint200,
...
proposal generation or optical tours. The isolated source-action override
calls inherited LocalRunner/TransferRunner once with selective=False and
stop_budget=0, preserving H's existing positive-version registration, then
runs endpoint handling. This reaches the frozen scalar executor once and
does not invoke inherited selective callbacks. Endpoint handler
...
Cover scheduling may invoke the endpoint handler twice at one physical stop:
once after scan_at returns and once through the frozen cover probe hook. This
is intentional. G shares its three tokens across both calls. For N+G, a real
positive G observation in the first call may enable N in the second call;
negative observations never renew permission, and failed-point exclusion and
the two-trial source cap still apply. Validate this interaction before any
N+G combination enters primary.

Current readiness: reviews/correction-readiness.md PASS_WITH_WARNINGS permits
corrected initial16-arm development. Historical trial data remain invalid.
...
review evaluates the corrected implementation and execution tests. In addition,
both isolated clear() and _try_clear_at() return without a physical call for
already-cleared targets, preventing a stale caller from resurrecting state.
