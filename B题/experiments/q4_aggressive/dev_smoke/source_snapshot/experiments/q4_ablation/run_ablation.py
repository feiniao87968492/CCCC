...
from q4_ablation_policies import VARIANTS, cover_context, make_runner
...
def capture_manifest(args, scenes, names):
    ...
    paths += [HERE/'q4_ablation_policies.py', Path(__file__), HERE/'design.md']
...
def main():
    ...
    changed = [p for p,d in manifest['sha256'].items()
               if not (ROOT/p).exists() or sha(ROOT/p) != d]
