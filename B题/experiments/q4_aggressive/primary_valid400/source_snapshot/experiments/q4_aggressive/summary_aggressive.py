...
def main(folder):
    ...
    if len(new):
        ...
        lines += ['', '## 新候选与尾部','',f'新候选最低均值为 {winner.label}：一般场景{winner.mean_T_over_K:.4f} s/源，全场景{all_winner.mean_T_over_K:.4f} s/源。',
                  f'独立边界压力组{stress_winner.cases}场：{stress_winner.mean_T_over_K:.4f} s/源，相对基线差{stress_winner.delta_vs_baseline:+.4f}，配对区间[{stress_winner.ci95_delta_low:.4f}, {stress_winner.ci95_delta_high:.4f}]。',
...
def render_table(table,folder,cohort,highlight):
    ...
    subtitle=f'每组清除 {int(base.source_total)} 个源；全场景清除与认证均通过' if complete else '存在未通过全场景清除与认证的策略，详见配套 CSV'
    ...
    meta={'source':source.name,'source_sha256':sha(source),'script_sha256':sha(__file__),'ranking':'all-setting eligibility then general non-boundary settings','highlight':highlight,'highlight_basis':'general-cohort minimum eligible mean, same highlight in all three figures'}
