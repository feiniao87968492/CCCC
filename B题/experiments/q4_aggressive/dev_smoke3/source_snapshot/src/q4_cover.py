...
COVER_HEX37 = "HEX37"
COVER_TRI25 = "TRI25"
_VALID_MODES = (COVER_SQUARE81, COVER_HEX37, COVER_TRI25)
...
def _build_tri25() -> CoverModel:
    """19-site 920 m lattice plus six 1950 m boundary witnesses.

    q4_certificate.discovery_certificate verifies the continuous discovery
    bound (< 944 m) independently of this fixed, offline routing heuristic.
    """
    step = 920.0
    pts = [(0.0, 0.0)] + [
        (step * (q + r / 2), step * np.sqrt(3) / 2 * r)
        for q in range(-2, 3) for r in range(-2, 3)
        if max(abs(q), abs(r), abs(q + r)) <= 2 and (q, r) != (0, 0)
    ]
    angles = np.arange(6) * np.pi / 3 + np.pi / 6
    pts = np.concatenate((pts, 1950 * np.column_stack((np.cos(angles), np.sin(angles)))))
    route = [0, 5, 1, 2, 21, 3, 6, 7, 20, 11, 10, 14, 13,
             9, 4, 22, 8, 12, 23, 16, 17, 24, 18, 15, 19]
    dist = np.linalg.norm(pts[:, None] - pts[None, :], axis=2)
    return CoverModel(COVER_TRI25, pts, _index_map(pts), route, dist, step, len(pts))


_SQUARE81 = _build_square81()
_HEX37 = _build_hex37()
_TRI25 = _build_tri25()
_MODELS = {COVER_SQUARE81: _SQUARE81, COVER_HEX37: _HEX37, COVER_TRI25: _TRI25}
...
def nearest_cover(y, mode: str | None = None) -> np.ndarray:
    ...
    if name == COVER_SQUARE81:
        return nearest_p4(y)
    if name == COVER_TRI25:
        return _TRI25.points[np.argmin(np.linalg.norm(_TRI25.points - y, axis=1))].copy()
    raise ValueError(f"unknown cover mode {mode!r}")
...
def covering_point(g, heading_rad: float, mode: str | None = None) -> np.ndarray:
    ...
    name = current_cover_mode() if mode is None else str(mode).strip().upper()
    if name == COVER_TRI25:
        pts = cover_points(name)
        vec = pts - g
        valid = (np.linalg.norm(vec, axis=1) < R_MIN) & (vec @ d > 0)
        return pts[np.flatnonzero(valid)[0]].copy()
