...
# 7b3. 图注超长检测（caption 只写简短标签：中文 >20 字 / 英文 >14 词 = 违规，判据/参数/结论移入正文）
#      同时覆盖 LaTeX \caption{...}（sections/*.tex、main.tex）与 docx ![alt](path)（main.md）两种语法。
echo "--- 图注超长检测 ---"
...
# --- LaTeX：平衡括号提取 \caption{...} ---
tex_files = []
sec = os.path.join(paper_dir, 'sections')
if os.path.isdir(sec):
    tex_files += [os.path.join(sec, fn) for fn in sorted(os.listdir(sec)) if fn.endswith('.tex')]
mt = os.path.join(paper_dir, 'main.tex')
if os.path.isfile(mt):
    tex_files.append(mt)
for f in tex_files:
    if re.search(r'appendix|附录|code|symbol', os.path.basename(f), re.I):
        continue
    try:
...
if bad:
    ...
    print(f'  共 {len(bad)} 处图注过长 — caption 只写简短标签，判据/参数/结论移入正文那段图解读里')
    sys.exit(3)
print('  OK: 图注长度合规')
PYEOF
)
...
# 14. Caption 长度检查（≤20 个中文字 / ≤12 个英文单词）
echo "--- Caption 长度检查 ---"
long_captions=0
for f in "$PAPER_DIR"/sections/*.tex "$PAPER_DIR"/main.tex; do
    [ -f "$f" ] || continue
...
for m in re.finditer(r'\\\\caption\{([^}]+)\}', content):
    ...
    elif zh_chars == 0 and len(cap.split()) > 12:
